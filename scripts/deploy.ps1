...
# 4. Initialise the franchise rights contract with the deployer as LGU admin.
Write-Host "Initialising franchise rights contract with admin=$Identity..."
try {
  stellar contract invoke --id $ContractId --source-account $Identity --network $Network -- init --admin $Identity
} catch {
